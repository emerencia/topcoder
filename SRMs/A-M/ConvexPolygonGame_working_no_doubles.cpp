      ...
      if (x1 == x2) {
        ...
        if (y2 <= y1) {
          ...
          for (int x=x1+1;x<x2;x++) {
            if (minn[x+kOff] == -1)
              ...
              minn[x+kOff] = min((LL)minn[x+kOff],
                                 y1+kOff+(((LL)x-x1)*(y2-y1))/(x2-x1));
          }
        } else {
          ...
          for (int x=x1+1;x<x2;x++) {
            if (minn[x+kOff] == -1)
              ...
              minn[x+kOff] = min((LL)minn[x+kOff],
                                 y1+kOff+(((LL)x-x1)*(y2-y1)+(x2-x1-1))/(x2-x1));
          }
        }
      } else {
        // we are at top
        ...
        if (y2 <= y1) {
          ...
          for (int x=x2+1;x<x1;x++)
            maxx[x+kOff] = max((LL)maxx[x+kOff],
                               y2+kOff+(((LL)x-x2)*(y1-y2))/(x1-x2));
        } else {
          // negative floor = (a-abs(b-1))/b
          for (int x=x2+1;x<x1;x++)
            maxx[x+kOff] = max((LL)maxx[x+kOff],
                               y2+kOff+(((LL)x-x2)*(y1-y2)-(x1-x2-1))/(x1-x2));
        }
      }
    }
