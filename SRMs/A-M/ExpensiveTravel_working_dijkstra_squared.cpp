...
class ExpensiveTravel {
 public:
  ...
  int minTime(vector <string> m, int startRow, int startCol, int endRow, int endCol) {
    ...
    qc.push(2521+2520/(m[startRow][startCol]-'0'));
    seen[startRow][startCol] = 2521+2520/(m[startRow][startCol]-'0');
    while (!q.empty()) {
      PII p = q.front();
      q.pop();
      int cost = qc.front();
      ...
      for (int k=0;k<4;k++) {
        ...
        if (ni >= 0 && ni < m.size() && nj >= 0 && nj < m[0].size() && m[ni][nj] != '1') {
          ...
          if (newcost/2521 > cost/2521)
            newcost = (newcost/2521)*2521+
                 2520/(m[ni][nj]-'0') + 
                 2520/(m[p.first][p.second]-'0');
    ...
    return seen[endRow][endCol] == -1 ? -1 : seen[endRow][endCol]/2521;
  }
};
