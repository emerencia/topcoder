...
class DonutsOnTheGrid {
 public:
  ...
  long long calc(int H, int W, int seed, int threshold) {
    ...
    vector<vector<int> > leftof(H,vector<int>(W,0));
    for (int i=0;i<H;i++)
      leftof[i][0] = grid[i][0];
    for (int j=1;j<W;j++)
      for (int i=0;i<H;i++)
        leftof[i][j] = grid[i][j] ? 1+leftof[i][j-1] : 0;
    vector<vector<LL> > next1(W,vector<LL>(W,0));
    vector<vector<LL> > next2(W,vector<LL>(W,0));
    vector<vector<LL> > tot(W,vector<LL>(W,0));
    LL r = 0;
    for (int i=0;i<H;i++) {
      for (int j=0;j<W;j++)
        if (!grid[i][j])
      ...
      next1 = next2;
      next2 = vector<vector<LL> >(W,vector<LL>(W,0));
    }
    return r;
  }
};
