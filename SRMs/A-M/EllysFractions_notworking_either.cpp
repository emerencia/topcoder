...
class EllysFractions {
 public:
  int N;
  vector<int> factors;
  void addfactors(int n, vector<int>& r) {
    for (int i=2;i*i<=n;i++) {
      while (n%i == 0) {
        r.push_back(i);
        n/=i;
  ...
  LL rec(int idx, LL A, LL B) {
    if (idx == factors.size()) return A < B;
    LL r = A < B;
    LL p = 1;
    for (int i=idx;i<factors.size() && factors[i] == factors[idx];i++)
      p*=factors[i];
    r+=rec(idx+1,A*p,B)+rec(idx+1,A,B*p);
    return r;
  }
