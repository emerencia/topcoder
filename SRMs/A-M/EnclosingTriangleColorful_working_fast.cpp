 ...
 public:
  ...
  int getNumber(int m, vector <int> x, vector <int> y) {
    ...
    for (int i=0;i<k*4;i++)
      ...
      long long c = 0;
    for (int y=3*k;y<4*k;y++)
      for (int r=2*k;r<3*k;r++) {
        if (!allleftofline[y][r]) continue;
        for (int j=0;j<2*k;j++) {
          c++;
          re += (allleftofline[j][y] & allleftofline[r][j]);
        }
      }
    for (int b=0;b<k;b++)
      for (int g=k;g<2*k;g++) {
        if (!allleftofline[g][b]) continue;
        for (int j=2*k;j<4*k;j++) {
          c++;
          re += (allleftofline[j][g] & allleftofline[b][j]);
        }
      }
    cout << c << endl;
    return re;
