...
class BagOfDevouring {
 public:
  ...
  double rec(int idx, int removed, int totalweight) {
    if (idx == values.size()) return 0;
    if (removed&(1<<idx)) return rec(idx+1, removed, totalweight);
    double t = rec(idx+1, removed, totalweight); // not removing
    double t2 = values[idx];
    removed |= (1<<idx);
    totalweight-= weights[idx];
    for (int i=0;i<values.size();i++) {
      if (removed&(1<<i)) continue;
      t2 += double(weights[i])/(totalweight+100)*rec(idx+1,removed|(1<<i),totalweight-weights[i]);
    }
    t2+= 100.0/(totalweight+100)*rec(idx+1,removed,totalweight);
    return max(t,t2);
  }
  double expectedYield(vector <int> values, vector <int> weights) {
    vector<PII> vws;
    for (int i=0;i<values.size();i++)
      vws.push_back(PII(values[i],weights[i]));
    sort(vws.rbegin(),vws.rend());
    vector<int> v, w;
    for (int i=0;i<vws.size();i++) {
      v.push_back(vws[i].first);
      w.push_back(vws[i].second);
    }
    this->values = v;
    this->weights = w;
    int totalweight = accumulate(weights.begin(),weights.end(),0);
    return rec(0,0,totalweight);
  }
};
