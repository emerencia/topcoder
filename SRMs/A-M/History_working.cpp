  ...
  string verifyClaims(vector <string> dynasties, vector <string> battles, vector <string> queries) {
    ...
    */
    for (int k=0;k<n;k++)
      for (int i=0;i<n;i++)
        for (int j=0;j<n;j++)
          if (minoff[i][k] != INT_MIN && minoff[k][j] != INT_MIN) {
            //if (dist[i][j] == -1 || dist[i][k]+dist[k][j] < dist[i][j]) {
              minoff[i][j] = max(minoff[i][j],minoff[i][k]+minoff[k][j]);
              maxoff[i][j] = min(maxoff[i][j],maxoff[i][k]+maxoff[k][j]);
              //dist[i][j] = dist[i][k]+dist[k][j];
            //}
          }
    ...
    for (int i=0;i<queries.size();i++) {
      ...
      if (minoff[dyn1][dyn2] == INT_MIN) continue;
