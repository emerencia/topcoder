...
class MysticAndCandiesEasy {
 public:
  vector<int> high;
  int maxtoinclude;
  int rec(int idx,int included) {
    int& r = mem[idx][included];
    if (r != -1) return r;
    if (idx == high.size()) {
      if (included > maxtoinclude) return r = INT_MIN;
      return r = 0;
    }
    int t = INT_MIN;
    t = max(t,1+rec(idx+1,included+high[idx]));
    t = max(t,rec(idx+1,included));
    return r = t;
  }
  int minBoxes(int C, int X, vector <int> high) {
    this->high = high;
    this->maxtoinclude = C-X;
    memset(mem,-1,sizeof(mem));
    return high.size()-rec(0,0);
  }
};
