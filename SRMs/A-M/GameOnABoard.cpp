 ...
 public:
  ...
  int optimalChoice(vector <string> costings) {
    ...
    for (int i=0;i<n*m;i++) {
      ...
      q.push(PII(-1*(costings[i/m][i%m] == '1'),i));
      ...
      while (!q.empty()) {
        ...
        q.pop();
        int mycost = -p.first;
        if (cost[p.second] != mycost) continue;
        int i = p.second/m;
        int j = p.second%m;
        ...
        for (int l=0;l<4;l++) {
          if (i+dx[l] >= 0 && i+dx[l] < n && j+dy[l] >= 0 && j+dy[l] < m) {
            ...
            int tcost = costings[i+dx[l]][j+dy[l]] == '1';
            if (cost[ni] == -1 || tcost+mycost < cost[ni]) {
              cost[ni] = tcost+mycost;
              q.push(PII(-(mycost+tcost),ni));
            }
          }
        }
      }
