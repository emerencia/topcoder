...
class EnclosingTriangleColorful {
 public:
  long long c;
  bool left_of_or_on_line(const PII& p, const PII& from, const PII& to) {
    c++;
    ...
    for (int i=0;i<2*k;i++) {
      for (int j=((i/k)+1)*k;j<3*k;j++) {
        ...
        for (int d=0;d<bl.size();d++)
          if (!left_of_or_on_line(bl[d],points[j],points[i])) {
            valid = false;
            break;
          }
        if (!valid) continue;
        for (int l=((j/k)+1)*k;l<4*k;l++) {
          ...
          bool val = true;
          for (int d=0;d<bl.size();d++)
            if (!left_of_or_on_line(bl[d],points[l],points[j]) ||
                !left_of_or_on_line(bl[d],points[i],points[l])) {
              val = false;
              break;
            }
          if (val) ++r;
