...
class Disaster {
 public:
  ...
  double rr;
  void rec(int from, int to, vector<int>& seen, double prob, int depth) {
    if (from == to) {
      double load = 1.0/(b*(depth+1));
      rr = max(rr,pow(1-b*load,depth)*prob*load);
      return;
    }
    seen[from] = true;
    for (int i=0;i<g.size();i++)
      if (g[from][i] != 0 && !seen[i])
        rec(i,to,seen,prob*g[from][i],depth+1);
    seen[from] = false;
  }
  double expected(double b, vector <string> roads) {
    this->b = b;
    g = vector<vector<double> >(roads.size(),vector<double>(roads.size(),0));
    ...
    vector<int> seen(roads.size(),false);
    rr = 0;
    rec(0,1,seen,1,0);
    return rr;
  }
