...
class LittleElephantAndRGB {
 public:
  long long getNumber(vector <string> list, int minGreen) {
    ...
    // this doesn't work because it doesn't say when it expires
    // dayum
    for (int a=0;a<N;a++) {
      int nrgreen = 0;
      bool iznice = false;
      ...
      for (int b=a;b+1<N;b++) {
       ...
       // if (!iznice && nrgreen) {
         ...
         // cout << a << " " << b << " " << nrgreen << endl;
          ...
          long long t = 0;
          for (int c=b+1;c<N;c++) {
            //cout << c << " " << minGreen-nrgreen << " .. " << dp[c][minGreen-nrgreen]  << " " << validconly[c] << " " << validc[c] << endl;
           //if (dp[c][minGreen-nrgreen] > validc[c])
              t+= dp[c][minGreen-nrgreen];// - validc[c]; // OR VALID DP
          if (nrgreen < minGreen)
            t+= dp[c][minGreen]; // it goes wrong because you don't count the ones that are valid
          }
          r+= t;
         }
      }
