...
class MysticAndCandies {
 public:
  int minBoxes(int C, int X, vector <int> low, vector <int> high) {
    // go over it twice, once by low and once by high
    int r = high.size();
    // first do the low ones
    ...
    int total_capacity = accumulate(high.begin(),high.end(),0);
    opened = 0;
    for (int i=0;i<high.size();i++) {
      if (total_capacity-opened <= C-X && opened >= X) {
        r = min(r, i);
        break;
      }
      opened += high[i];
