...
// BEGIN CUT HERE
const int tt = 4;
// 20:40: rewrite with memoization
// END CUT HERE

class EllysCandyGame {
 public:
  map<vector<int>,int> mem;
  int rec(vector<int>& sweets) {
    if (mem.count(sweets))
      return mem[sweets];
    int r = INT_MIN;
    for (int i=0;i<sweets.size();i++) {
      if (sweets[i] == 0) continue;
      int olda1 = INT_MIN, olda2, olda3 = INT_MIN;
      if (i > 0) {
        olda1 = sweets[i-1];
        sweets[i-1] *= 2;
      }
      ...
      olda2 = sweets[i];
      sweets[i] = 0;
      r = max(r,olda2-rec(sweets));
      sweets[i] = olda2;
      if (olda1 != INT_MIN) sweets[i-1] = olda1;
      if (olda3 != INT_MIN) sweets[i+1] = olda3;
    }
    if (r == INT_MIN) return mem[sweets] = 0;
    return mem[sweets]=r;
  }
  string getWinner(vector <int> sweets) {
    ...
    int r = rec(sweets);
    cout << mem.size() << endl;
    for (map<vector<int>,int>::iterator i=mem.begin();i!=mem.end();i++) {
