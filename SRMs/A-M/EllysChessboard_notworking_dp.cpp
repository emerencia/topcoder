 ...
 public:
  ...
  int calccost(long long curmask, long long addmask) {
    ...
    for (int i=0;i<n*n;i++) {
      ...
      for (int j=0;j<n*n;j++) {
        ...
        mx = max(mx,abs((i/n)-(j/n))+abs((i%n)-(j%n)));
      }
      r+= mx;
    }
    return r;
  ...
  long long insidemask(int sr,int sc,int nr,int nc) {
    ...
    for (int i=sr;i < sr+nr;i++)
      for (int j=sc;j < sc+nc;j++)
        r |= (1LL<<(i*n+j));
    return r;
  }
  int minCost(vector <string> board) {
    n = 8;
    fill(&dp[0][0][0][0],&dp[8][0][0][0],2000);
    dp[0][0][n][n] = 0;
    int r = INT_MAX;
    long long stonemask = 0;
    for (int i=0;i<n;i++)
      for (int j=0;j<n;j++)
        if (board[i][j] == '#')
          stonemask |= (1LL<<(i*n+j));
        ...
        for (int sc=0;sc<n;sc++)
          for (int nc=n-sc;nc>0;nc--) {
            ...
            for (int k=0;k<4;k++) {
              ...
              if (omask) {
                int addedcost = calccost(curmask&stonemask,(curmask^omask)&stonemask);
                dp[sr][sc][nr][nc] = min(dp[sr][sc][nr][nc],
                    dp[sr+srs[k]][sc+scs[k]][nr+nrs[k]][nc+ncs[k]]+addedcost);
              }
            }
            if (bitcount(curmask&stonemask) < 2)
