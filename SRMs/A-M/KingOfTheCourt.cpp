...
class KingOfTheCourt {
 public:
  ...
  double chancesOfWinning(vector <int> ability) {
    ...
    int cur=0,next=1;
    for (int steps=MAX_STEPS;steps>=0;steps--) {
      for (int state=0;state<c;state++)
        for (int consecwins=0;consecwins<=ability.size()-1;consecwins++) {
          int permut = num_to_permut[state];
          int king  = permut%10;
          if (steps == MAX_STEPS) {
            mem[state][consecwins][cur] = chancewin;
            continue;
          }
          if (consecwins == ability.size()-1) {
            mem[state][consecwins][cur] = king == 0;
            continue;
          }
          ...
          mem[state][consecwins][cur] = 0;
          mem[state][consecwins][cur] += (1-problose)*mem[nextstate][consecwins+1][next];
          // king loses
          nextpermut = nextplayer + (permut/10 - (permut/10)%10) + factor*king;
          nextstate = permut_to_num[nextpermut];
          mem[state][consecwins][cur] += problose*mem[nextstate][0][next];
        }
      swap(cur,next);
    }
    return mem[0][0][cur];
