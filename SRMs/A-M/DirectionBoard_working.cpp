...
class DirectionBoard {
 public:
  int getMinimum(vector <string> board) {
    ...
    //int SINK = 2*n*m;
    //int SOURCE = SINK+1;
    //NetworkFlow* f = new NetworkFlow(SOURCE+1);
    // it works like this:
    ...
    for (int i=0;i<n;i++)
      for (int j=0;j<m;j++) {
        ...
        //f->addEdge(SOURCE,i*m+j,1);
        addedge(n*m+i*m+j,dest,1,0);
        //f->addEdge(n*m+i*m+j,SINK,1);
      }
    for (int i=0;i<n;i++)
      for (int j=0;j<m;j++) {
        for (int k=0;k<4;k++) {
          ...
          addedge(i*m+j,n*m+ni*m+nj,1,g[i][j] == k ? 0 : 1);
        }
      }
    int c1,c2;
    //return f->maxFlow(SOURCE,SINK);
    minCostMaxFlow(c1,c2);
    return c1; // maxflow is the same as what i had earlier
  }
};
