...
class EllysRoomAssignmentsDiv1 {
 public:
  double getAverage(vector <string> ratings) {
    ...
    int e2 = 0;
    double d2 =0;
    bool ellyinlast = false;
    while (c < rating.size()) {
      double a = 0;
      bool ellyinthis = false;
      int b = 0;
      bool f = false;
      for (int i=0;i<R;i++) {
        if (rating[c] == elly) { ellyinthis = true; }
        a += rating[c++];
        b++;
        if (c == rating.size()) { f = true; break; }
      }
      a /= b;
      if (ellyinthis) a = elly;
      if (f) {
        e2 = e+1;
        d2 = d+a;
        ellyinlast = ellyinthis;
      } else {
        e++;
        d+=a;
      }
    ...
    d2/=e2;
    //cout << elly << " " << N << " " << R << endl;
    if (rating.size()%R == 0) return d2;
    if (ellyinlast) return d2;
    double frac = ((rating.size()%R))/double(R);
    //cout << frac << endl;
    return d2*frac+d*(1-frac);
  }
...
// BEGIN CUT HERE
	...
	void verify_case(int Case, const double &Expected, const double &Received) { cerr << "Test Case #" << Case << "..."; if (abs(Expected - Received) < 0.0001) cerr << "PASSED" << endl; else { cerr << "FAILED" << endl; cerr << "\tExpected: \"" << Expected << '\"' << endl; cerr << "\tReceived: \"" << Received << '\"' << endl; } }
