      ...
      if (x1 == x2) {
        ...
        if (y2 <= y1) {
          ...
          for (int x=x1+1;x<x2;x++)
            minn[x+kOff] = y1+kOff+(((LL)x-x1)*(y2-y1))/(x2-x1);
        } else {
          // positive ceil = (a+abs(b-1))/b
          for (int x=x1+1;x<x2;x++)
            minn[x+kOff] = y1+kOff+(((LL)x-x1)*(y2-y1)+(x2-x1-1))/(x2-x1);
        }
      } else {
        // we are at top
        maxx[x1+kOff] = y1-1+kOff;
        maxx[x2+kOff] = y2-1+kOff;
        if (y2 <= y1) {
          // positive floor = a/b
          for (int x=x2+1;x<x1;x++)
            maxx[x+kOff] = y2+kOff+(((LL)x-x2)*(y1-y2))/(x1-x2);
        } else {
          // negative floor = (a-abs(b-1))/b
          for (int x=x2+1;x<x1;x++)
            maxx[x+kOff] = y2+kOff+(((LL)x-x2)*(y1-y2)-(x1-x2-1))/(x1-x2);
        }
      }
    }
    for (int i=0;i<minn.size();i++) {
      if (minn[i] == -1 || maxx[i] == -1 || minn[i] > maxx[i]) continue;
