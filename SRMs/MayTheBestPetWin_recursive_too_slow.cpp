...
class MayTheBestPetWin {
 public:
  ...
  int rec(int idx,int sum) {
    if (idx == n) {
      return max(sum-totA,totB-sum);
    }
    if (mem.count(PII(idx,sum))) return mem[PII(idx,sum)];
    return mem[PII(idx,sum)] = min(rec(idx+1,sum),rec(idx+1,sum+A[idx]+B[idx]));
  }
  int calc(vector <int> A, vector <int> B) {
    this->A = A;
    this->B = B;
    n = A.size();
    totA = accumulate(A.begin(),A.end(),0);
    totB = accumulate(B.begin(),B.end(),0);
    mem = map<PII,int>();
    return rec(0,0);
  }
