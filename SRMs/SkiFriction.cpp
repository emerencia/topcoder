  ...
  int bestPosition(string skiFriction, string pathFriction) {
    ...
    for (int i=0;i<pathFriction.size()-skiFriction.size();i++) {
      ...
      for (int j=0;j<skiFriction.size();j++)
        mx = max(mx,skiFriction[j]-'0'+pathFriction[i+j]-'0');
      r+=mx;
    }
    return r;
  }
...
int main(int argc, char* argv[])
{
	...
	{
		cout << "Testing SkiFriction (250.0 points)" << endl << endl;
		for (int i = 0; i < 20; i++)
		{
	...
	else
	{
		...
		switch (_tc)
		{
			case 0:
			{
				...
				_expected = 33;
				_received = _obj.bestPosition(skiFriction, pathFriction); break;
			}
			case 1:
			{
