...
class PilingRectsDiv1 {
 public:
  ...
  typedef pair<Rectangle,int> PRI;
  bool operator()(const PRI& a, const PRI& b) const {
  ...
  long long getmax(int N, vector <int> XS, vector <int> YS, int XA, int XB, int XC, int YA, int YB, int YC) {
    ...
    LL mx = 0;
    vector<PRI> rectangles2;
    for (int i=2;i<rectangles.size();i++)
      rectangles2.push_back(PRI(rectangles[i],i));
    ...
    for (int i=0;i<N;i++) {
      ...
      for (int j=0;j<rectangles2.size();j++)
        if (rectangles2[j].second > i+1) {
          miny = min(miny,rectangles2[j].first.y);
          if (idx == N-1) nmin1 = rectangles2[j].first.y;
          if (idx == N-i-1) nminimin1 = rectangles2[j].first.y;
          idx++;
        }
      ly = min(ly,miny);
      ry = min(ry,nminimin1);
