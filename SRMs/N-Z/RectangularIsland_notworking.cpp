...
class RectangularIsland {
 public:
  double theProbablity(int width, int height, int x, int y, int steps) {
    queue<PII> q;
    queue<int> qs; // steps
    vector<vector<LL> > weight(width,vector<LL>(height,1));
    vector<vector<LL> > newweight = weight;
    q.push(PII(x,y));
    qs.push(0);
    int r = 0;
    int oldsteps = -1;
    while (!q.empty()) {
      PII p = q.front(); q.pop();
      int step = qs.front(); qs.pop();
      if (step != oldsteps) {
        oldsteps = step;
        weight = newweight;
        newweight = vector<vector<LL> >(width,vector<LL>(height,0));
      }
      if (step == steps) {
        r+=weight[p.first][p.second];
        continue;
      }
      for (int k=0;k<4;k++) {
        int ni = p.first+dx[k];
        ...
        if (ni >= 0 && ni < width && nj >= 0 && nj < height) {
          if (newweight[ni][nj] == 0) {
            q.push(PII(ni, nj));
            qs.push(step + 1);
          }
          newweight[ni][nj]+=weight[p.first][p.second];
        }
      }
    }
    cout << r << endl;
