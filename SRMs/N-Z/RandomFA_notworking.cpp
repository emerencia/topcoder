...
class RandomFA {
 public:
  ...
  int finalState;
  double rec(int state, int togo) {
    double& r = mem[state][togo];
    if (seen[state][togo]) return r;
    seen[state][togo] = 1;
    if (togo == 0) return r = state == finalState;
    if (state == 51) return r = state == finalState;
    r = state == finalState;
    double r2 = 0;
    for (int j=0;j<rulesa[state].size();j++) {
      if (!rulesa[state][j]) continue;
      r2 += 1.0/3*rulesa[state][j]/100.0*rec(j,togo-1);
    }
    for (int j=0;j<rulesb[state].size();j++) {
      if (!rulesb[state][j]) continue;
      r2 += 1.0/3*rulesb[state][j]/100.0*rec(j,togo-1);
    }
    for (int j=0;j<rulesc[state].size();j++) {
      if (!rulesc[state][j]) continue;
      r2 += 1.0/3*rulesc[state][j]/100.0*rec(j,togo-1);
    }
    r+=r2;
    return r;
  }
