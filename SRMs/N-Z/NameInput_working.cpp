...
class NameInput {
 public:
  ...
  int rec(int idx, int pos) {
    ...
    int t = INF;
    char c = name[idx];
    int idx1 = predict.find(c,pos);
    if (idx1 == string::npos) idx1 = predict.find(c);
    int d = min((pos-idx1+n)%n,(idx1-pos+n)%n);
    t = min(t,d+rec(idx+1,idx1));
    int idx2 = predict.find_last_of(c,pos);
    if (idx2 == string::npos) idx2 = predict.find_last_of(c,n-1);
    d = min((pos-idx2+n)%n,(idx2-pos+n)%n);
    t = min(t,d+rec(idx+1,idx2));
    return r = t;
  }
  int countUpDownKeyPresses(vector <string> predictionSequence, vector <string> nname) {
    ...
    m = name.size();
    for (int i=0;i<m;i++)
      if (predict.find(name[i]) == string::npos)
        return -1;
