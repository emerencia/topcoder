  ...
  int rec(int ai, int bi, int ci) {
    ...
    if (ci == C.size()) {
      if (ai == A.size() || bi == B.size())
        return r = 0;
      if (A[ai] == B[bi])
        return r = 1+rec(ai+1,bi+1,ci);
      return r = max(rec(ai+1,bi,ci),rec(ai,bi+1,ci));
    }
    if (ai == A.size() || bi == B.size())
      return r = -100000;
    int t = -100000;
    if (A[ai] == B[bi] && ci == 0)
      t = 1+rec(ai+1,bi+1,ci);
    if (A[ai] == B[bi] && B[bi] == C[ci])
      t = max(t,1+rec(ai+1,bi+1,ci+1));
    //if (A[ai] != B[bi])
      t = max(t,max(rec(ai+1,bi,ci),rec(ai,bi+1,ci)));
    return r = t;
  }
