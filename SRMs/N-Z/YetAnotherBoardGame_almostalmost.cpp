...
class YetAnotherBoardGame {
 public:
  int N;
  int M;
  int minmoveSub(int q,vector<vector<bool> > board, int flips, vector<int> rowtype, vector<int> coltype) {
    int state = 0;
    for (int i=0;i<M;i++)
      if (board[0][i])
        state |= (1<<(M-i-1));
  ...
  int minimumMoves(vector <string> board) {
    ...
    for (int i=0;i<(1<<M);i++)
      for (int movetype=1;movetype<3;movetype++) {
        ...
        r = mymin(r,minmoveSub(0,b,flips,rowtype,coltype));
        r = mymin(r,minmoveSub(1,b,flips,rowtype,coltype));
      }
    return r;
  }
