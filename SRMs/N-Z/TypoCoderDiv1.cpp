...
// END CUT HERE
const int kInf = -1000;
map<PILL,long long> mem;
class TypoCoderDiv1 {
 public:
  vector<int> D;
  int rec(long long cur, int idx) {
    if (idx == D.size()) return 0;
    if (mem.count(PILL(cur,idx))) return mem[PILL(cur,idx)];
    if (cur >= 2200) {
      int diff = min(cur,(LL)D[idx]);
      int nxt = cur-diff;
      if (nxt >= 2200) return kInf;
      return mem[PILL(cur,idx)] = 1+rec(nxt,idx+1);
    }
    // rating less
    return mem[PILL(cur,idx)] = max(((cur+D[idx] >= 2200) ? 1 : 0) + rec(cur+D[idx],idx+1),rec(cur-min(cur,(LL)D[idx]),idx+1));
  }
  int getmax(vector <int> d, int X) {
    // X < 2200
    D = d;
    mem = map<PILL,long long>();
    int r = rec(X,0);
    return (r < -102 ? 0 : r);
  }
// BEGIN CUT HERE
