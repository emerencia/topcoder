...
class PowerOfThree {
 public:
  ...
  bool rec(int steps, int state, long long xx, long long yy) {
    //cout << xx << " " << yy << endl;
    ...
    if (x != xx) {
      if (xx + pow(3,steps) <= x)
        ans |= rec(steps+1,state|(1<<steps),xx+pow(3,steps),yy);
      if (!ans)
        ans |= rec(steps+1,state|(1<<steps),xx-pow(3,steps),yy);
    }
    if (!ans && y != yy) {
      if (yy + pow(3,steps) <= y)
        ans |= rec(steps+1,state,xx,yy+pow(3,steps));
      if (!ans)
        ans |= rec(steps+1,state,xx,yy-pow(3,steps));
    }
  ...
  string ableToGet(int x, int y) {
    ...
    this->x = abs(x);
    this->y = abs(y);
    return (rec(0,0,0,0) ? "Possible" : "Impossible");
  }
