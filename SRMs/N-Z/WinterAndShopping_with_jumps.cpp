  ...
  LL rec(int idx, int lr, int lg, int rr, int rg) {
    ...
    */
    ...
    if (cl[idx] != -1 && cr[idx] != -1) {
      // find whichever one closes first
      if (last[cl[idx]] <= last[cr[idx]]) {
        // l closes first
        if (rr < lr || rg < lg || rb < lb) return 0;
        (t+=(((bico(lr+lg+lb,lr)*bico(lg+lb,lg))%kMod)*
            rec(last[cl[idx]]+1,0,0,rr-lr,rg-lg))%kMod)%=kMod;
      } else {
        // r closes first
        if (lr < rr || lg < rg || lb < rb) return 0;
        (t+=(((bico(rr+rg+rb,rr)*bico(rg+rb,rg))%kMod)*
            rec(last[cr[idx]]+1,lr-rr,lg-rg,0,0))%kMod)%=kMod;
      }
      /*
