...
class YetAnotherTwoTeamsProblem {
 public:
  long long count(vector <int> skill) {
    ...
    long long r = 0;
    vector<LL> dp(totalsum/2+2,0);
    dp[0] = 1;
    for (int j=0;j<skill.size();j++)
      for (int k=dp.size()-1;k>=skill[j];k--)
        dp[k] += dp[k-skill[j]];
    for (int i=0;i<skill.size();i++) {
      int halfremainder = totalsum/2;
      int start = max(0,halfremainder-skill[i]+1);
      int maxsize = totalsum-halfremainder;
      for (int k=skill[i];k<dp.size();k++)
        dp[k] -= dp[k-skill[i]];
