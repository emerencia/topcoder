  ...
  int play(vector <string> costs) {
    ...
    for (int k=0;k<N*M;k++) {
      for (int i=0;i<M*N;i++) {
        int ii = i/M;
        int jj = i%M;
        if (costs[ii][jj] == 'x') continue;
        if (ii) {
          int j = 0;
          if (dp[(ii-1)*M+jj][0] != -1)
            dp[i][j] = max(dp[i][j],costs[ii][jj]-'0'+dp[(ii-1)*M+jj][0]);
          if (dp[(ii-1)*M+jj][1] != -1)
            dp[i][j] = max(dp[i][j],costs[ii][jj]-'0'+dp[(ii-1)*M+jj][1]);
          if (dp[(ii-1)*M+jj][2] != -1)
            dp[i][j] = max(dp[i][j],costs[ii][jj]-'0'+dp[(ii-1)*M+jj][2]);
        }
        if (jj) {
          int j = 1;
          if (dp[ii*M+(jj-1)][0] != -1)
            dp[i][j] = max(dp[i][j],costs[ii][jj]-'0'+dp[ii*M+(jj-1)][0]);
          if (dp[ii*M+(jj-1)][1] != -1)
            dp[i][j] = max(dp[i][j],costs[ii][jj]-'0'+dp[ii*M+(jj-1)][1]);
        }
        if (jj+1 < M) {
          int j = 2;
          if (dp[ii*M+(jj+1)][0] != -1)
            dp[i][j] = max(dp[i][j],costs[ii][jj]-'0'+dp[ii*M+(jj+1)][0]);
          if (dp[ii*M+(jj+1)][2] != -1)
            dp[i][j] = max(dp[i][j],costs[ii][jj]-'0'+dp[ii*M+(jj+1)][2]);
        }
      }
    }
    int mx = 0;
    for (int i=0;i<M;i++)
      mx = max(mx,dp[(N-1)*M+i][0]);
    return mx;
  }
