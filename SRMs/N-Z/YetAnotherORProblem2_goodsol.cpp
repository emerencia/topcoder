...
class YetAnotherORProblem2 {
 public:
  ...
  int countSequences(int N, int R) {
    long long r = 1;
    for (int i = 1;i <= R; i++)
     (r+=modPow(N,1+zerobits(i)))%=kMod; // the current number is new, so any combination with it is also new. wherever there is room (zero bit), one of the other combinations can have a 1, which adds to the combinations
    // the current number is the highest number. all combinations are new and valid (because we only add 1s to zeros inside the current number)
    // all combinations are unique because they include at least one number (the current number) that hasn't been seen before
    // we treats all combinations because we treat all combinations of each number
    return r;
  }
