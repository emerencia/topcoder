...
class PieOrDolphin {
 public:
  vector <int> Distribute(vector <int> choice1, vector <int> choice2) {
    vector<vector<queue<int> > > g = vector<vector<queue<int> > >(50, vector<queue<int> >(50));
    vector<int> degree(50,0);
    for (int i = 0;i<choice1.size();i++) {
      g[choice1[i]][choice2[i]].push(i);
      degree[choice1[i]]++;
      degree[choice2[i]]++;
    }
    int edgecount = choice1.size();
    vector<int> r(choice1.size(),0);
    vector<int> balance(50,0);
    while (edgecount) {
      bool changed = false;
      for (int i = 0;i < 50;i++)
        for (int j = 0;j < 50;j++) {
          ...
          if ((balance[i] == -1 && balance[j] == -1) ||
              (balance[i] == 1 && balance[j] == 1)) continue;
          changed = true;
          edgecount--;
          int edge = g[i][j].front();
          g[i][j].pop();
          if (balance[i] == -1 && balance[j] == 1) {
            r[edge] = 1; balance[i]++; balance[j]--;
          } else if (balance[i] == -1 && balance[j] == 0) {
            r[edge] = 1; balance[i]++; balance[j]--;
          } else if (balance[i] == 0 && balance[j] == -1) {
            r[edge] = 2; balance[i]--; balance[j]++;
          } else if (balance[i] == 0 && balance[j] == 0) {
            // could be either one
            r[edge] = 2; balance[i]--; balance[j]++;
          } else if (balance[i] == 0 && balance[j] == 1) {
            r[edge] = 1; balance[i]++; balance[j]--;
          } else if (balance[i] == 1 && balance[j] == -1) {
            r[edge] = 2; balance[i]--; balance[j]++;
          } else if (balance[i] == 1 && balance[j] == 0) {
            r[edge] = 2; balance[i]--; balance[j]++;
          } else {
            cout << "warning: invalid state found" << endl;
          }
        }

      if (!changed) {
        cout << "warning: we found a loop where nothing changed" << endl;
        break;
      }
    }
    return r;
