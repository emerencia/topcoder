...
class WinterAndShopping {
 public:
  ...
  vector<int> cl;
  vector<int> cr;
  LL rec(int idx, int lr, int lg, int lb, int rr, int rg, int rb) {
    /*
    cout << idx << " "  <<
        lr << " "  <<
        lg << " "  <<
    ...
    */  
    if (idx == 501) {
      if (lr == 0 && lg == 0 && lb == 0 && rr == 0 && rg == 0 && rb == 0)
        return 1;
      return 0;
    }
    if (add[0][idx] != -1) {
    ...
    if (cl[idx] != -1 && cr[idx] != -1) {
      if (lr && rr) (t+= rec(idx+1,lr-1,lg,lb,rr-1,rg,rb))%=kMod;
      if (lg && rg) (t+= rec(idx+1,lr,lg-1,lb,rr,rg-1,rb))%=kMod;
      if (lb && rb) (t+= rec(idx+1,lr,lg,lb-1,rr,rg,rb-1))%=kMod;
    } else if (cl[idx] != -1) {
      if (lr) (t+= rec(idx+1,lr-1,lg,lb,rr,rg,rb))%=kMod;
      if (lg) (t+= rec(idx+1,lr,lg-1,lb,rr,rg,rb))%=kMod;
      if (lb) (t+= rec(idx+1,lr,lg,lb-1,rr,rg,rb))%=kMod;
    } else if (cr[idx] != -1) {
      if (rr) (t+= rec(idx+1,lr,lg,lb,rr-1,rg,rb))%=kMod;
      if (rg) (t+= rec(idx+1,lr,lg,lb,rr,rg-1,rb))%=kMod;
      if (rb) (t+= rec(idx+1,lr,lg,lb,rr,rg,rb-1))%=kMod;
    } else {
      (t+=rec(idx+1,lr,lg,lb,rr,rg,rb))%=kMod;
    }
    return t;
  }
