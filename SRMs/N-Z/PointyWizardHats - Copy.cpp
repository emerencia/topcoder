...
// do SRM 549 http://apps.topcoder.com/wiki/display/tc/SRM+549
class PointyWizardHats {
 public:
   int mx;
   int ths;
   void rec(const vector<vector<int> >& compatible,vector<bool>& chosen,int idx,int c) {
     for (int i=0;i<chosen.size();i++) {
       if (chosen[compatible[idx][i]]) continue;
       chosen[compatible[idx][i]] = true;
       mx = max(mx,c+1);
       rec(compatible,chosen,compatible[idx][i],c+1);
       chosen[compatible[idx][i]] = false;
     }
   }
  int getNumHats(vector <int> topHeight, vector <int> topRadius, vector <int> bottomHeight, vector <int> bottomRadius) {
    ths = topHeight.size()+bottomHeight.size();
    vector<vector<int> > compatible(topHeight.size()+bottomHeight.size());
    for (int i=0;i<topHeight.size();i++) {
      for (int j=0;j<bottomHeight.size();j++) {
        if (topRadius[i] < bottomRadius[j] && 
          ...
          compatible[i].push_back(topHeight.size()+j);
          compatible[topHeight.size()+j].push_back(i);
        }
      }
    }
    int r = 0;
    vector<bool> chosen(compatible.size(),false);
    for (int i=0;i<compatible.size();i++) {
      chosen[i] = true;
      mx = 0;
      rec(compatible,chosen,i,0);
      r+= (mx+1)/2;
    }
    return r;
  }
