...
class PowerOfThree {
 public:
  ...
  bool rec2(int steps, int state, long long yy) {
    if (mem2.count(PII(steps,yy))) return mem2[PII(steps,yy)];
    if (y == yy) {
      r2 = true;
      return mem2[PII(steps,yy)] = true;
    }
    if (steps > 15) return mem2[PII(steps,yy)] = false;
    bool ans = false;
    if (!(ystate&(1<<steps)) && yy + poww[steps] <= y)
      ans |= rec2(steps+1,state|(1<<steps),yy+poww[steps]);
    if (!ans && !(ystate&(1<<steps)))
      ans |= rec2(steps+1,state|(1<<steps),yy-poww[steps]);
    if (!ans && (ystate&(1<<steps)))
      ans |= rec2(steps+1,state,yy);
    return mem2[PII(steps,yy)]=ans;
  }
  bool rec(int steps, int state, long long xx) {
    //cout << xx << " " << yy << endl;
    ...
    if (steps > 15) return mem[PII(steps,xx)] = false;
    bool ans = false;
    if (xx + pow(3,steps) <= x)
      ans |= rec(steps+1,state|(1<<steps),xx+poww[steps]);
    if (!ans)
      ans |= rec(steps+1,state|(1<<steps),xx-poww[steps]);
    if (!ans)
      ans |= rec(steps+1,state,xx);
