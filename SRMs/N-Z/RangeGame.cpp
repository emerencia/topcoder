...
class RangeGame {
 public:
  ...
  PII rec(int idx, const vector<Pattern>& patterns, Pattern cur) {
    if (idx == patterns.size()) return PII(0,-cur.first());
    PII r = rec(idx+1,patterns,cur);
    if (patterns[idx].matches(cur)) {
      PII r2 = rec(idx+1,patterns,patterns[idx].intersect(cur));
      r = max(r,PII(1+r2.first,r2.second));
    }
    return r;
  }
  int most_likely(const vector<Pattern>& patterns) {
    Pattern a;
    PII r = rec(0,patterns,a);
    return -r.second;
  }
