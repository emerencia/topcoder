    ...
    if (cur == 0) {
      ...
      for (int i=0;i<words.size();i++) {
        ...
        if (s.size() < words[i].size()) {
          ...
          // sizes equal
          if (reverseeq(s,words[i])) {
            t+=rec(state|(1<<i),2,0,0);
          }
        }
      }
    } else if (cur == 1) {
      ...
      // cur = 2, no current elem selected
      if (state) t+=1;
      for (int i=0;i<words.size();i++)
        if (palindromicwords[i] && (!(state&(1<<i))))
          t++;
      for (int i=0;i<words.size();i++) {
        if (state&(1<<i)) continue;
        for (int j=0;j<words.size();j++) {
          if (i == j || (state&(1<<j))) continue;
          int sizediff = abs((int)words[i].size()-(int)words[j].size());
          if (words[i].size() < words[j].size()) {
            // continue on right side
            if (reverseeq(words[i],words[j].substr(sizediff))) {
              t+=rec(state|(1<<i)|(1<<j),1,sizediff,j);
            }
          } else if (words[j].size() < words[i].size()) {
            // continue with left word
            int offset = words[j].size();
            if (reverseeq(words[i].substr(0,offset),words[j])) {
              t+=rec(state|(1<<i)|(1<<j),0,offset,i);
            }
          } else {
            // sizes equal
            if (reverseeq(words[i],words[j])) {
              t+=rec(state|(1<<i)|(1<<j),2,0,0);
            }
          }
        }
      }
