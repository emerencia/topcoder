 ...
 public:
  ...
  LL rec(int minhour, int idx) {
    ...
    if (idx == maxhour.size())
      return r = 1;
    LL t = 0;
    if (minhour > maxhour[idx]) return r = 0;
    for (int hour=minhour;hour<=maxhour[idx];hour++) {
      int nextminhour = hour;
      if (idx+1 != maxhour.size() && minuteValues[idx] >= minuteValues[idx+1])
        nextminhour++;
  ...
  vector <int> hourValues(vector <int> minuteValues, int k) {
    ...
    LL K = k-1;
    maxhour = vector<int>(minuteValues.size());
    int curhour = 23;
    maxhour[minuteValues.size()-1] = curhour;
    for (int i=(int)minuteValues.size()-2;i>=0;i--) {
      if (minuteValues[i] >= minuteValues[i+1]) {
        curhour--;
        if (curhour < 0) return vector<int>(1,-1);
      }
      maxhour[i] = curhour;
    }
    int minhour = 0;
    ...
    for (int i=0;i<minuteValues.size();i++) {
      LL option = 0;
      for (int hour = minhour;hour<=maxhour[i];hour++) {
        int ghour = hour;
