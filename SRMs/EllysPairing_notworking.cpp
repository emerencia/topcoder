...
class EllysPairing {
 public:
  int check_add(int &cnt, int &last, int val) {
    if (last == -1) {
      cnt = 1;
      last = val;
      return 0;
  ...
  int getMax(int M, vector <int> count, vector <int> first, vector <int> mult, vector <int> add) {
    ...
    for (int i=0;i<count.size();i++) {
      ...
      vector<PII> pv;
      for (map<int,int>::iterator i=mem.begin();i!=mem.end();i++)
        pv.push_back(PII(-i->second,i->first));
      sort(pv.begin(), pv.end());
      for (int j=0;j<pv.size();j++)
        for (int i=0;i<abs(pv[j].first);i++)
          r += check_add(cnt, last, pv[j].second);
    }
    //cout << "vals left: " << cnt << endl;
    return r;
  }
};
