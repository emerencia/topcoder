...
// There is a maximum of min(N,M) unknown flips until all flips are determined
// and then it's just linear til the end. Complexity O(2^26) ?
...
class YetAnotherBoardGame {
 public:
  ...
  int minimumMoves(vector <string> board) {
    ...
    for (int i=0;i<(1<<M);i++)
      r = mymin(r,mmsubrec(0,0,i,b,rowtype,coltype));
    return r;
  }
